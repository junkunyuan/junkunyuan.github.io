...
"""
<ol>
    <li> <b>Find a research topic.</b> Read recent papers and answer: (1) <i>What is the value of this topic?</i> (2) <i>Are there any well-known researchers or institutions involved in this topic?</i> (Selecting a topic is crucial. By aligning with veterans, one can reduce the risk of pursuing topics with limited value, especially for early-stage researchers.) &nbsp;&nbsp;  Some paper sources: <a href="https://proceedings.neurips.cc//">NeurIPS</a>, <a href="https://openreview.net/group?id=ICLR.cc">ICLR</a>, <a href="https://proceedings.mlr.press/">ICML</a>, <a href="https://openaccess.thecvf.com/menu">CVPR & ICCV & ECCV</a>, and <a href="https://aclanthology.org/">ACL & EMNLP</a>.
...
<ul>
    ...
    <li> <b>Reproduce baseline results.</b> Run the codebase on the selected dataset and verify that you can reproduce the results reported in prior works. This step is <i>very important</i>, so ensure you can either reproduce the results or provide a well-reasoned explanation if reproduction fails.
    ...
    <li> <b>Document experiments.</b> Maintain an experiment log and update it whenever you <i>start</i> or <i>complete</i> a new experiment. <a href="resource/else/experiment progress example.xlsx">Example document</a>.
</ul>
fig: fig1.png 800
...
<b>There is no one-size-fits-all format for paper writing.</b> The following guidance is based solely on my personal experience. Everyone can develop unique and creative ideas for their papers based on their own thinking/insights and the distinctiveness of each research project.
...
<ul>
    ...
    <li> <b>Method.</b> Clearly present your proposed method by first explaining the high-level motivation and idea. An effective paper should be able to communicate its methods and results primarily through figures and tables—akin to a silent presentation—without requiring readers to scrutinize every word of the main text. <b>Note:</b> do not overcomplicate your method (such as overusing ineffective symbols or formulas) for sophistication's sake—clarity, simplicity, effectiveness, and scalablity are key values in AI research (think about ResNet, Dropout, Mixup, YOLO, etc.).
